...
async function exportToDocx(filename, content) {
  ...
  // Simple parser to turn Markdown tables into actual Word document tables
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("|") && line.endsWith("|")) {
      inTable = true;
      // Skip the markdown table divider lines (e.g. |---|---|)
  ...
  if (inTable) children.push(new Table({ rows: tableRows, width: { size: 100, type: WidthType.PERCENTAGE } }));
